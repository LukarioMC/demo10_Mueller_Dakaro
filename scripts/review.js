...
db.collection("Hikes").where("id", "==", hikeID).get()
    .then(queryHike => {
        // See how many results you got from the query
        size = queryHike.size;
        // Get the documents in the query
        Hikes = queryHike.docs;

        // We want to have one document per hike, so if the the result of 
        // the query greater than one, we can check it and clean the DB if needed.
        if (size = 1) {
